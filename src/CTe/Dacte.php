...
class Dacte extends DaCommon
{
    ...
    protected $arrNF = array();
    protected $arrayNFe = array();
    protected $arrInfOutros = array();
    protected $arrIdDocAntEle = array();
    protected $arrInfCTeMultimodal = array();
    protected $totPag;
    protected $margemInterna = 0;
    protected $formatoChave = "#### #### #### #### #### #### #### #### #### #### ####";
    protected $cteSub;
    protected $yDocOriginal;
    ...
    private function loadDoc($xml)
    {
        ...
        if (!empty($xml)) {
            ...
            $this->textoAdic = $this->dom->getElementsByTagName("ObsCont");
    ...
    protected function monta(
        ...
        // FINAL DO BLOCO DE REMETENTE / DESTINATARIO / EXPEDIDOR / RECEBEDOR
        $r = $this->tomador($x, $y);
    ...
    protected function cabecalho($x = 0, $y = 0, $pag = '1', $totPag = '1')
    {
        ...
        $oldY += $h;
        //desenha a caixa
        $this->pdf->textBox($x, $y, $w + 2, $h + 1, '', $aFont, 'T', 'L', 0, '');
        // coloca o logo
        ...
        $h1 = 17.5;
        $y1 = $y + $h - 2;
        $this->pdf->textBox($x, $y1, $w + 2, $h1, '', $aFont, 'T', 'L', 0, '');
        //TIPO DO CT-E
        $texto = 'TIPO DO CTE';
        //$wa = 37;
        $wa = 34;
        ...
        $this->pdf->textBox($x, $y1, $w * 0.5, $h1, $texto, $aFont, 'T', 'L', 0, '');
        ...
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($x, $y1 + 3, $w * 0.5, $h1, $texto, $aFont, 'T', 'L', 0, '', false);
        //TIPO DO SERVIÇO
        ...
        $this->pdf->textBox($x + $wa, $y1, $w * 0.5, $h1, $texto, $aFont, 'T', 'L', 0, '');
        ...
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($x + $wa, $y1 + 3, $w * 0.5, $h1, $texto, $aFont, 'T', 'L', 0, '', false);
        //TOMADOR DO SERVIÇO
        ...
        $this->pdf->textBox($x, $y2, $w * 0.5, $h1, $texto, $aFont, 'T', 'L', 0, '');
        ...
        if ($toma == 1) {
            ...
            $this->pdf->textBox($x - 14, $y2 + 3.5, $w * 0.5, $h1, 'X', $aFont, 'T', 'C', 0, '', false);
        } else {
            $aFont = array(
                'font' => $this->fontePadrao,
                'size' => 11,
            ...
            $this->pdf->textBox($x - 2, $y2 + 3.5, $w * 0.5, $h1, 'X', $aFont, 'T', 'C', 0, '', false);
        }
        $aFont = $this->formatNegrito;
        // QUADRADO DO SIM E NÃO.
        ...
        $this->pdf->Line($x + 15, $y2 + 8, $x + 20, $y2 + 8);
        $this->pdf->textBox($x + 4, $y2 + 1.4 + 3, $w * 0.5, $h1, 'NÃO', $aFont, 'T', 'C', 0, '', false);
        //FORMA DE PAGAMENTO
        ...
        $w1 = $w;
        $h = 8;
        // Quadrado em torno do dacte  VER AQUI PARA MUDAR A PARTE DE CIMA DEPOIS !!!
        $this->pdf->textBox($x, $y, $w, $h + 1, '', $aFont, 'T', 'L', 0, '');
        $texto = "DACTE";
        $aFont = array(
            'font' => $this->fontePadrao,
            'size' => $this->default_size,
        ...
        $w2 = $w;
        $h = 8;
        $this->pdf->textBox($x1, $y, $w - 0.5, $h + 1, '', $aFont, 'T', 'L', 0, '');
        $texto = "MODAL";
        $aFont = array(
            'font' => $this->fontePadrao,
            'size' => $this->default_size,
        ...
        $this->pdf->textBox($x1, $y + 1, $w, $h, $texto, $aFont, 'T', 'L', 0, '');
        //$texto = '1/1';
        $texto = $pag . "/" . $totPag;
        $aFont = array(
            'font' => $this->fontePadrao,
        ...
        $this->pdf->textBox($x1, $y + 5, $w, $h, $texto, $aFont, 'T', 'L', 0, '');
        ...
        $h = 7;
        $w = round($maxW * 0.16);
        $this->pdf->textBox($x, $y, $w, $h + 1, "", $aFont, 'T', 'C', 0, '', false, 0, 0, true);
        //modelo
        $wa = 12;
        $xa = $x;
        $texto = 'MODELO';
        ...
        $xa = $x + $w + 1;
        $wa =  32;
        $this->pdf->textBox($xa, $y, $wa, $h + 1, '', $aFont, 'T', 'C', 0, '', false, 0, 0, true);
        ...
        $h1 = 14;
        $w = round($maxW * .444);
        $this->pdf->textBox($x, $y, $w + 0.5, $h1, '', $aFont, 'T', 'C', 0, '');
        //CODIGO DE BARRAS
        ...
        $this->pdf->setFillColor(255, 255, 255);
        $this->pdf->textBox($x, $y + $h1, $w, $h, $texto, $aFont, 'T', 'C', 0, '', true, 0, 0, false);
        $aFont = array(
            'font' => $this->fontePadrao,
            'size' => 12,
            'style' => 'B'
        ...
        $this->pdf->textBox($x, $y + $h1 + 3, $w, $h + 6, $texto, $aFont, 'T', 'C', 0, '');
        ...
        $this->pdf->textBox($x, $y - 2 + $h1 + 11, $w, $h, $texto, $aFont, 'T', 'C', 0, '', true, 0, 0, false);
        //outra caixa
        $y += $h + 6;
        $h = 8.5;
        $wa = $w;
        ...
        $this->pdf->textBox($x, $y + 3, $wa, $h, $texto, $aFont, 'T', 'C', 0, '', true, 0, 0, false);
        if (!empty($this->numdepec)) {
            $texto = $this->numdepec;
        ...
        $this->pdf->textBox($x, $y1, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        ...
        $this->pdf->textBox($x, $y1 + 3.5, $w, 3, $texto, $aFont, 'T', 'L', 0, '', false, 0, 0, false);
        //ORIGEM DA PRESTAÇÃO
        $y += $h + 6.5;
        $x = $oldX;
        $h = 8;
        ...
        if (!$resp['status']) {
            ...
            if (!empty($texto)) {
                $y += 3;
                $h = 5;
                $aFont = ['font' => $this->fontePadrao, 'size' => $this->default_size, 'style' => 'B'];
    ...
    protected function rodape($x, $y)
    {
        ...
        $this->pdf->textBox($x, $y, $w, 4, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $texto = '';
    ...
    protected function remetente($x = 0, $y = 0)
    {
        ...
        $y = $y - 0.5;
        $this->pdf->textBox($x1, $y, $w - 18, $h, $texto, $aFont, 'T', 'L', 0, '', true);
        $y += 3;
        $texto = 'ENDEREÇO';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        ...
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($x1, $y - .5, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $x = $w - 40;
        $texto = 'CEP';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        ...
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($x1, $y - .5, $w, $h, $cpfCnpj, $aFont, 'T', 'L', 0, '');
        $x = $w - 40;
        $texto = 'IE';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        ...
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $texto = $this->getTagValue($this->rem, "UF") ?? '';
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($x + 4, $y, 5, 3, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);

        $texto = 'PAÍS';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x1, $y, $w - 10, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $texto = $this->getTagValue($this->rem, "xPais") != "" ?
            $this->getTagValue($this->rem, "xPais") : 'BRASIL';
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($x1 + 6, $y, $w - 6, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $x = $w - 40;
        $texto = 'FONE';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
    ...
    protected function destinatario($x = 0, $y = 0)
    {
        ...
        $texto = 'DESTINATÁRIO';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', false, '');
        $aFont = $this->formatNegrito;
        $texto = $this->getTagValue($this->dest, "xNome");
        $this->pdf->textBox($x1, $y - .5, $w - 18, $h, $texto, $aFont, 'T', 'L', false, '', true);
        $y += 3;
        $texto = 'ENDEREÇO';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        ...
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $texto = $this->getTagValue($this->dest, "UF") ?? '';
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($x + 4, $y, 5, 3, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        ...
        $texto = $this->getTagValue($this->dest, "xPais") != "" ?
            $this->getTagValue($this->dest, "xPais") : 'BRASIL';
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($x1 + 6, $y, $w - 6, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $x = $w + $w - 40;
        $texto = 'FONE';
    ...
    protected function expedidor($x = 0, $y = 0)
    {
        ...
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $texto = $this->getTagValue($this->exped, "UF") ?? '';
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($x + 4, $y, 5, 3, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        ...
        if (isset($this->exped)) {
            $texto = $this->getTagValue($this->exped, "xPais") != "" ?
                $this->getTagValue($this->exped, "xPais") : 'BRASIL';
        } else {
            $texto = '';
        }
        $aFont = $this->formatNegrito;
    ...
    protected function recebedor($x = 0, $y = 0)
    {
        ...
        $texto = 'RECEBEDOR';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x - 0.5, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $aFont = $this->formatNegrito;
        ...
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $cpfCnpj = $this->formatCNPJCPF($this->receb);
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($x1, $y - .5, $w, $h, $cpfCnpj, $aFont, 'T', 'L', 0, '');
        $x = $w + $w - 40;
        $texto = 'IE';
        ...
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $texto = $this->getTagValue($this->receb, "UF") ?? '';
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($x + 4, $y, 5, 3, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        ...
        if (isset($this->receb)) {
            $texto = $this->getTagValue($this->receb, "xPais") != "" ?
                $this->getTagValue($this->receb, "xPais") : 'BRASIL';
        } else {
            $texto = '';
        }
        $aFont = $this->formatNegrito;
    ...
    protected function tomador($x = 0, $y = 0)
    {
        ...
        $this->pdf->textBox($x + 29, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $x = $maxW * 0.60;
        $texto = 'MUNICÍPIO';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        ...
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($x + 15, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $x = $maxW * 0.85;
        $texto = 'UF';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        ...
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($x + 4, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $x = $w - 18;
        $texto = 'CEP';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        ...
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($x + 6, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $y += 3;
        $x = $oldX;
        $texto = 'ENDEREÇO';
        $aFont = $this->formatPadrao;
        ...
        $this->pdf->textBox($x + 16, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $y += 3;
        $texto = 'CNPJ/CPF';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        ...
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($x + 13, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $x = $x + 65;
        ...
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($x + 28, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $x = $w * 0.75;
        $texto = 'PAÍS';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
    ...
    protected function descricaoCarga($x = 0, $y = 0)
    {
        ...
        $this->pdf->textBox($x + 8, $y + 3, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $x = $w * 0.12;

        // FAZER VARIÁVEL DAQUI ATÉ
        $texto = "";
        $texto2 = "";
        foreach ($this->infQ as $infQ) {
        ...
        $this->pdf->textBox($x + 20, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        ...
        $this->pdf->textBox($x + 20, $y + 3, $w, $h, $texto2, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $x = $w * 0.24;
        ...
        $this->pdf->textBox($x + 35, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $qCarga = 0;
        foreach ($this->infQ as $infQ) {
        ...
        $this->pdf->textBox($x + 35, $y + 3, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $x = $w * 0.36;
        // AQUI
        $texto = 'CUBAGEM(M3)';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x + 60, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $qCarga = 0;
        foreach ($this->infQ as $infQ) {
        ...
        $texto = 'QTDE(VOL)';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x + 83, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $qCarga = 0;
        foreach ($this->infQ as $infQ) {
        ...
        $this->pdf->textBox($x + 83, $y + 3, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $x = $w * 0.53;
        ...
        $aFont = array(
            ...
            'size' => $this->default_size,
            'style' => 'B');
        $this->pdf->textBox($x + 31, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $y += 3;
        $texto = 'RESPONSÁVEL';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
    ...
    protected function compValorServ($x = 0, $y = 0)
    {
        ...
        $this->pdf->textBox($x, $y + 4, $w * 0.17, $h, $texto, $aFont, 'T', 'R', 0, '');
        $y += 10;
        $y += 1;
    ...
    protected function impostos($x = 0, $y = 0)
    {
        ...
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w * 0.26, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $x += $w * 0.26;
        ...
        $texto = 'ALÍQ ICMS';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w * $wCol02, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $x += $w * $wCol02;
        $texto = 'VALOR ICMS';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w * $wCol02, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        ...
        $texto = 'VALOR ICMS ST';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x + 6, $y, $w * $wCol02, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        /*$x += $w * 0.14;
        $texto = 'ICMS ST';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w * 0.14, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
    ...
    protected function detCont($x = 0, $y = 0)
    {
        ...
        $this->pdf->textBox($x, $y, $w, $h, $texto, $this->formatPadrao, 'T', 'C', 0, '');
        $y += 3.4;
        $h = 29.6;
        $y += 1;
        ...
        $texto = $descr1;
        $this->pdf->textBox($x, $y, $w * 0.10, $h, $texto, $this->formatPadrao, 'T', 'L', 0, '', true, 0, 0, false);
        $x += $w * 0.07;
        $texto = $descr2;
        $this->pdf->textBox($x, $y, $w * 0.23, $h, $texto, $this->formatPadrao, 'T', 'L', 0, '', true, 0, 0, false);
        $x += $w * 0.28;
        $texto = $descr3;
        $this->pdf->textBox($x, $y, $w * 0.13, $h, $texto, $this->formatPadrao, 'T', 'L', 0, '', true, 0, 0, false);
        $x += $w * 0.14;
        $texto = $descr4;
        $this->pdf->textBox($x, $y, $w * 0.23, $h, $texto, $this->formatPadrao, 'T', 'L', 0, '', true, 0, 0, false);
        $x += $w * 0.20;
        ...
        foreach ($this->detCont as $k => $d) {
            ...
            if ($qtdCont < 8) {
                $this->pdf->textBox($x - $w * 0.2, $yCont, $w * 0.23, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
            }
            ...
            foreach ($infNF as $i => $z) {
                ...
                if ($totalNF > 7) {
                    ...
                    $texto = $tp;
                    $this->pdf->textBox($auxX, $yIniDados, $w * 0.23, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
                    //$auxX += $w * 0.09;
                    $auxX += $w * 0.07;
                    $texto = $cnpj;
                    $this->pdf->textBox($auxX, $yIniDados, $w * 0.23, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
                    $auxX += $w * 0.28;
                    $texto = $doc;
                    $this->pdf->textBox($auxX, $yIniDados, $w * 0.13, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
    ...
    protected function detContContinuacao($x = 0, $y = 0)
    {
        ...
        for ($i = 2; $i <= $this->totPag; $i++) {
            ...
            foreach ($this->arrayCont as $conteiner) {
                ...
                $this->pdf->textBox($x, $yCont, $w * 0.23, $h, $nCont, $this->formatPadrao, 'T', 'L', 0, '');
                $texto = $conteiner['nLacre'];
                $this->pdf->textBox($x - $w * 0.2, $yCont, $w * 0.23, $h, $texto, $this->formatPadrao, 'T', 'L', 0, '', true, 0, 0, false);
                $yCont += 3;
            }
    ...
    protected function docOrig($x = 0, $y = 0)
    {
        ...
        $texto = $descr2;
        $this->pdf->textBox($x, $y, $w * 0.23, $h, $texto, $this->formatPadrao, 'T', 'L', 0, '', true, 0, 0, false);
        $x += $w * 0.27; // COLUNA SÉRIE/NRO.DOCUMENTO DA DIREITA
        $texto = $descr3;
        $this->pdf->textBox($x, $y, $w * 0.13, $h, $texto, $this->formatPadrao, 'T', 'L', 0, '', true, 0, 0, false);

        $auxX = $oldX;
        $this->yDocOriginal = $y + 3;
        $iniy = $y + 3.5;
        ...
        foreach ($this->infNF as $k => $d) {
            $this->arrNF[] = $this->infNF->item($k);
        }

        foreach ($this->infOutros as $k => $d) {
            $this->arrInfOutros[] = $this->infOutros->item($k);
        }
        ...
        $contador = 0;
        $qtdeDocs = count($this->arrayNFe) + count($this->arrNF) + count($this->arrInfOutros) + count($this->arrIdDocAntEle) + count($this->arrInfCTeMultimodal);
        if ($qtdeDocs > 16) {
            // Max numero de paginas é 11 para > 1060 documentos
            $this->totPag = min(floor(($qtdeDocs - 16 - 1) / 116) + 2, 11);
            $this->flagDocOrigContinuacao = 1;
        } else {
            $this->totPag = 1;
        }

        for ($i = 0; $i < count($this->arrNF); $i++, $contador++) {
            if ($contador == 16) {
                break;
            }

            $mod = $this->arrNF[$i]->getElementsByTagName('mod');
            $tp = ($mod && $mod->length > 0) ? $mod->item(0)->nodeValue : '';
            $cnpj = $this->formatCNPJCPF($this->rem);
            $doc = $this->arrNF[$i]->getElementsByTagName('serie')->item(0)->nodeValue;
            $doc .= '/' . $this->arrNF[$i]->getElementsByTagName('nDoc')->item(0)->nodeValue;
            if ($auxX > $w * 0.90) {
                $this->yDocOriginal = $this->yDocOriginal + 3;
                $auxX = $oldX;
            }
        ...
        //$r = $this->cabecalho(1, 1, '1', $this->totPag);
        for ($i = 0; $i < count($this->arrayNFe); $i++, $contador++) {
            if ($contador == 16) {
                break;
            }

            $tp = 'NF-e';
            $chaveNFe = $this->arrayNFe[$i];
            $numNFe = substr($chaveNFe, 25, 9);
            $serieNFe = substr($chaveNFe, 22, 3);
            $doc = $serieNFe . '/' . $numNFe;
            if ($auxX > $w * 0.90) {
                $this->yDocOriginal = $this->yDocOriginal + 3.5;
                $auxX = $oldX;
            }
            ...
            $auxX += $w * 0.09;
            $texto = $chaveNFe;
            $this->pdf->textBox($auxX, $this->yDocOriginal, $w * 0.27, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
            $auxX += $w * 0.28;
            $texto = $doc;
        ...
        for ($i = 0; $i < count($this->arrInfOutros); $i++, $contador++) {
            if ($contador == 16) {
                break;
            }

            $tpDoc = $this->getTagValue($this->arrInfOutros[$i], "tpDoc");
            $descOutros = $this->getTagValue($this->arrInfOutros[$i], "descOutros");
            $nDoc = $this->getTagValue($this->arrInfOutros[$i], "nDoc");
            $dEmi = $this->getTagValue($this->arrInfOutros[$i], "dEmi");
            if (!empty($dEmi)) {
                $tempData = (new \DateTime($this->getTagValue($this->arrInfOutros[$i], "dEmi")))->format('d/m/Y');
                $dEmi = "Emissão: " . $tempData;
            }
            $vDocFisc = $this->getTagValue($this->arrInfOutros[$i], "vDocFisc", "Valor: ");
            $dPrev = $this->getTagValue($this->arrInfOutros[$i], "dPrev");
            if (!empty($dPrev)) {
                $tempData = (new \DateTime($this->getTagValue($this->arrInfOutros[$i], "dPrev")))->format('d/m/Y');
        ...
        for ($i = 0; $i < count($this->arrIdDocAntEle); $i++, $contador++) {
            if ($contador == 16) {
                break;
            }

            $tp = 'CT-e';
            $chaveCTe = $this->arrIdDocAntEle[$i];
        ...
        for ($i = 0; $i < count($this->arrInfCTeMultimodal); $i++, $contador++) {
            if ($contador == 16) {
                break;
            }

            $tp = 'CT-e';
            $chaveCTe = $this->arrInfCTeMultimodal[$i];
            $numCTe = substr($chaveCTe, 25, 9);
            $serieCTe = substr($chaveCTe, 22, 3);
            $doc = $serieCTe . '/' . $numCTe;
            if ($auxX > $w * 0.90) {
        ...
        $this->pdf->line($maxW * .5, $this->yDocOriginal + 3.5, $maxW * .5, $iniy);
    }
    ...
    protected function docOrigContinuacao($x = 0, $y = 0)
    {
        ...
        $nNF = count($this->arrNF);
        $nNFe = count($this->arrayNFe);
        $nInfOutros =  count($this->arrInfOutros);
        $nIdDocAntEle = count($this->arrIdDocAntEle);
        $nInfCTeMultimodal =  count($this->arrInfCTeMultimodal);
        $nDocs = $nNF + $nNFe + $nInfOutros + $nIdDocAntEle + $nInfCTeMultimodal;
        ...
        for ($page = 2; $page <= $this->totPag; $page++) {
            $x = $x2;
            $y = $y2;
            $this->pdf->AddPage($this->orientacao, $this->papel);
            $this->pdf->setMargins(5, 5, 5);
            $r = $this->cabecalho($x, 3, $page, $this->totPag);
            $oldX = $x;
            ...
            $qtd = 0;
            if ($contador < $nNF) {
                for ($i = $contador - $qtd; $i < $nNF; $i++, $contador++) {
                    if ($contador % ((116 * ($page - 1)) + 16) == 0) {
                        break;
                    }
            ...
            $qtd += $nNF;
            if ($contador < $qtd + $nNFe) {
                for ($i = $contador - $qtd; $i < $nNFe; $i++, $contador++) {
                    if ($contador % ((116 * ($page - 1)) + 16) == 0) {
                        break;
                    }
            ...
            $qtd += $nNFe;
            if ($contador < $qtd + $nInfOutros) {
                for ($i = $contador - $qtd; $i < $nInfOutros; $i++, $contador++) {
                    if ($contador % ((116 * ($page - 1)) + 16) == 0) {
                        break;
                    }
                    ...
                    $this->pdf->textBox($auxX, $yIniDados, $w * 0.09, $h, $tpDoc, $aFont, 'T', 'L', 0, '', false);
                    $auxX += $w * 0.09;
            ...
            $qtd += $nInfOutros;
            if ($contador < $qtd + $nIdDocAntEle) {
                //echo "nIdDocAntEle {$nIdDocAntEle} {$qtd} {$contador} {$page}";
                for ($i = $contador - $qtd; $i < $nIdDocAntEle; $i++, $contador++) {
                    if ($contador % ((116 * ($page - 1)) + 16) == 0) {
                        break;
                    }
            ...
            $qtd += $nIdDocAntEle;
            if ($contador < $qtd + $nInfCTeMultimodal) {
                for ($i = $contador - $qtd; $i < $nInfCTeMultimodal; $i++, $contador++) {
                    if ($contador % ((116 * ($page - 1)) + 16) == 0) {
                        break;
                    }
    ...
    protected function docCompl($x = 0, $y = 0)
    {
        ...
        if ($this->cteSub != null) {
            if ($this->cteSub->getElementsByTagName('chCte')->length && $this->cteSub->getElementsByTagName('chCte')->item(0)->nodeValue) {
                ...
                $this->pdf->textBox($w * 0.40, $yIniDados, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
                $yIniDados += 3.5;
            }
    ...
    protected function modalAereo($x = 0, $y = 0)
    {
        ...
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x + 50, $y, $w * 0.23, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $texto = 'CÓDIGO DA TARIFA';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x + 80, $y, $w * 0.23, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $texto = 'VALOR DA TARIFA';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x + 110, $y, $w * 0.23, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $texto = $this->getTagValue($this->aereo, "nOCA");
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($x, $y + 3, $w * 0.23, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $x += $w * 0.23;
    ...
    protected function modalMultimodal($x = 0, $y = 0)
    {
        ...
        $aFont = $this->formatPadrao;
        $x = $oldX;
        $this->pdf->textBox($x, $y, $w, $h * 1.2, $texto, $aFont, 'T', 'C', 0, '');
        $y += 3.4;
        ...
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w * 0.7, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $multimodal = $this->dom->getElementsByTagName("multimodal")->item(0);
        $texto = $this->getTagValue($multimodal, 'COTM');
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($x, $y + 3, $w * 0.50, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $x += $w * 0.7;
        ...
        $texto = 'NEGOCIÁVEL';
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($x + ($w * 0.04), $y + 3, $w * 0.50, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $x += $w * 0.15;


        $texto = 'NÃO NEGOCIÁVEL';
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($x + ($w * 0.03), $y + 3, $w * 0.50, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $x += 6;
        $x = $oldX;
        $y += $h - 1.5;
        ...
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($oldX, $y + 3.5, $w * 0.33, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $segMultimodal = $multimodal->getElementsByTagName("seg")->item(0);
        $cnpjSeg = $this->getTagValue($segMultimodal, 'CNPJ');
        $texto = $cnpjSeg;
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($oldX, $y + 7, $w * 0.33, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $texto = 'NOME DA SEGURADORA';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($oldX + ($w * 0.25), $y + 3.5, $w * 0.25, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $texto = $this->getTagValue($segMultimodal, 'xSeg');
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($oldX + ($w * 0.25), $y + 7, $w * 0.25, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $texto = 'NÚMERO DA APÓLICE';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($oldX + ($w * 0.50), $y + 3.5, $w * 0.25, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $texto = $this->getTagValue($multimodal, 'nApol');
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($oldX + ($w * 0.50), $y + 7, $w * 0.25, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $texto = 'NÚMERO DA AVERBAÇÃO';
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($oldX + ($w * 0.75), $y + 3.5, $w * 0.25, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $texto = $this->getTagValue($multimodal, 'nAver');
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($oldX + ($w * 0.75), $y + 7, $w * 0.25, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
    }
    ...
    protected function modalAquaviario($x = 0, $y = 0)
    {
        ...
        if (!empty($this->detCont)) {
            ...
            $aFont = $this->formatPadrao;
            $this->pdf->textBox($x, $y, $w, $h * 1.3, $texto, $aFont, 'T', 'C', 0, '');
            $y += 3.4;

            $texto = 'LACRE';
            $aFont = $this->formatPadrao;
            $this->pdf->textBox($x, $y, $w * 0.33, $h + 7.5, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
            $x += $w * 0.50;

            $texto = 'IDENTIFICAÇÃO DO CONTAINER';
            $aFont = $this->formatPadrao;
            $this->pdf->textBox($x, $y, $w * 0.33, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
            $y += 7.5;
        } else {
            $y += 27.5;
        }
        ...
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w * 0.23, $h * 2, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $texto = '';
    ...
    protected function modalFerr($x = 0, $y = 0)
    {
        ...
        $this->pdf->textBox($x, $y + 3, $w * 0.10, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $x += $w * 0.06;
        $y1 = $y + 12.5;
        $texto = 'NUM';
        $aFont = array(
        ...
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $wa, $h1, $texto, $aFont, 'T', 'C', 0, '');
        ...
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x, $y, $w - 21.5, $h1 * 2.019, $texto, $aFont, 'T', 'C', 0, '');
        $y += 3.4;
        $w = $w * 0.2;
        $h = $h * 1.04;
        ...
        $this->pdf->textBox($x, $y + 6, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        ...
        $this->pdf->textBox($x, $y + 9, $w, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $x += 50;
        $texto = 'NOME';
        $aFont = array(
            'font' => $this->fontePadrao,
    ...
    protected function canhoto($x = 0, $y = 0)
    {
        ...
        $this->pdf->textBox($x, $y, $w * 0.25, $h, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, false);
        $x += $w * 0.25;
        ...
        $texto = "CT-E";
        $aFont = $this->formatNegrito;
        $this->pdf->textBox($x + 10, $y - 5, $w * 0.15, $h, $texto, $aFont, 'T', 'L', 0, '');
        $numCTe = str_pad($this->getTagValue($this->ide, "nCT"), 9, '0', STR_PAD_LEFT);
        $texto = "Número: " . (int)$numCTe . "\n";
        ...
        $this->pdf->textBox($x + 10, $y - 10, $w * 0.15, $h, $texto, $aFont, 'C', 'L', 0, '');
        //        $texto .= "Série      {$numSerie}";
        $numSerie = str_pad($this->getTagValue($this->ide, "serie"), 3, '0', STR_PAD_LEFT);
        $texto = "Série:      " . $this->getTagValue($this->ide, "serie");
        $aFont = array(
            'font' => $this->fontePadrao,
            'size' => $this->default_size,
            'style' => 'b'
        );
        $this->pdf->textBox($x + 10, $y - 7, $w * 0.15, $h, $texto, $aFont, 'C', 'L', 0, '');
    ...
    protected function dadosAdic($x, $y, $pag, $h)
    {
        ...
        $this->pdf->textBox($x, $y, 60, $h, $texto, $aFont, 'T', 'L', 0, '');
        ...
        $y += 2;
        $aFont = $this->formatPadrao;
        $this->pdf->textBox($x + 1, $y + 2, $w + 70, $h + 5, $texto, $aFont, 'T', 'L', 0, '', true, 0, 0, true, false);
        return $y + $h;
    }
